...
export async function onRequest(context) {
  ...
  const moviesPromise = fetch(`https://tmdb-proxy.sujeetunbeatable.workers.dev/3/trending/movie/week?api_key=${TMDB_API_KEY}`)
    .then(r => r.json())
    .catch(() => ({ results: [] }));

  const tvShowsPromise = fetch(`https://tmdb-proxy.sujeetunbeatable.workers.dev/3/trending/tv/week?api_key=${TMDB_API_KEY}`)
    .then(r => r.json())
    .catch(() => ({ results: [] }));
